	...
	public void onCreate(Bundle savedInstanceState) {
		...
		btnAction.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				/**
				 * Obtenemos los botones de imagen completa y de galería para
				 * revisar su estatus más adelante
				 */
				...
				/**
				 * Para todos los casos es necesario un intent, si accesamos la
				 * cámara con la acción ACTION_IMAGE_CAPTURE, si accesamos la
				 * galería con la acción ACTION_PICK. En el caso de la vista
				 * previa (thumbnail) no se necesita más que el intent, el
				 * código e iniciar la actividad. Por eso inicializamos las
				 * variables intent y code con los valores necesarios para el
				 * caso del thumbnail, así si ese es el botón seleccionado no
				 * validamos nada en un if.
				 */
				Intent intent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
				int code = TAKE_PICTURE;

				/**
				 * Si la opción seleccionada es fotografía completa, necesitamos
				 * un archivo donde guardarla
				 */
				if (rbtnFull.isChecked()) {
					Uri output = Uri.fromFile(new File(name));
					intent.putExtra(MediaStore.EXTRA_OUTPUT, output);
					/**
					 * Si la opción seleccionada es ir a la galería, el intent
					 * es diferente y el código también, en la consecuencia de
					 * que esté chequeado el botón de la galería se hacen esas
					 * asignaciones
					 */
				...
				/**
				 * Luego, con todo preparado iniciamos la actividad
				 * correspondiente.
				 */
